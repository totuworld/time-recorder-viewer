...
import { IAddOverWork, IAddOverWorkData } from './interface/IAddOverWork';
...
export class TimeRecord {
  ...
  public static extractEtcTime(
    ...
    const filteredValues = Object.keys(childData)
      .filter(fv => childData[fv].type === target)
      .map(mv => childData[mv])
      .reduce<{
        time: number;
        lastWorkTimeStamp: string;
        timeObj: luxon.DurationObject;
      }>(
        (acc, cur: ITimeRecordLogData) => {
          if (!!cur.done) {
            const duration = Util.getBetweenDuration(cur.time, cur.done);
            acc.time += duration.as('hours');
            const updateTimeObj = luxon.Duration.fromObject(acc.timeObj)
              .plus(duration)
              .toObject();
  ...
  public static extractRestTime(childData: {
    [key: string]: ITimeRecordLogData;
  }) {
    return TimeRecord.extractEtcTime(childData, EN_WORK_TYPE.REST);
  }

  public static extractEmergencyTime(childData: {
    [key: string]: ITimeRecordLogData;
  }) {
  ...
  public static extractWorkTime(
    ...
    const workFilter = Object.keys(childData)
      .filter(
        fv => childData[fv].type === startType || childData[fv].type === endType
      )
      .map(mv => childData[mv])
      .reduce<{
        time: number;
  ...
  public static convertWorkTime(
    ...
    const updateDatas =
      value.length > 0
        ? value.map(mv => {
            ...
            const remoteTime = TimeRecord.extractWorkTime(
              mv[dateStr],
              EN_WORK_TYPE.REMOTE,
              EN_WORK_TYPE.REMOTEDONE
            );
            const restTime = TimeRecord.extractRestTime(mv[dateStr]);
            const emergencyTime = TimeRecord.extractEmergencyTime(mv[dateStr]);
            const vacationTime = TimeRecord.extractEtcTime(
              mv[dateStr],
              EN_WORK_TYPE.VACATION
            );
            const overLoadTime = TimeRecord.extractEtcTime(
              mv[dateStr],
              EN_WORK_TYPE.FUSEOVERLOAD
            );
    ...
    const totalWorkTimeStr = Util.reduceDurationObject(
      timeObjs,
      EN_WORK_TYPE.WORK
    ).toFormat('hh:mm:ss');
    const totalLawRestTimeStr = Util.reduceDurationObject(
      timeLawRestObjs,
      EN_WORK_TYPE.REST
    ).toFormat('hh:mm:ss');
    const totalRestTimeStr = Util.reduceDurationObject(
      timeObjs,
      EN_WORK_TYPE.REST
    ).toFormat('hh:mm:ss');
    const totalEmergencyTimeStr = Util.reduceDurationObject(
      timeObjs,
      EN_WORK_TYPE.EMERGENCY
    ).toFormat('hh:mm:ss');
    const totalRemoteTimeStr = Util.reduceDurationObject(
      timeObjs,
      EN_WORK_TYPE.REMOTE
    ).toFormat('hh:mm:ss');
    ...
    const calWorkTimeStr = luxon.Duration.fromObject(calWorkTimeObj).toFormat(
      'hh:mm:ss'
    );
    let range =
      startDate === endDate
  ...
  public static checkAddWorkType(
    logs: ITimeRecordLogData[],
    targetType: EN_WORK_TYPE
  ) {
    if (
  ...
  public static possibleAddRest(logs: ITimeRecordLogData[]) {
    const remoteLogs = this.reduceWorkLogs(logs);
    const haveActiveRest =
      logs.filter(fv => fv.type === EN_WORK_TYPE.REST && !!fv.done === false)
        .length > 0;
    // work나 remote가 열려있고, 완료되지 않은 rest가 없을 때 가능.
    return (
      (remoteLogs.WORK === true || remoteLogs.REMOTE === true) &&
      haveActiveRest === false
    );
  }
  ...
  public async addOverWorkByUser(
    params: AddOverloadRecordReqParam,
    schema: IJSONSchemaType
  ): Promise<IAddOverWork> {
    ...
    const query = this.rb.postOverWorkQuery({
      method: 'POST',
      headers: {},
      body: validParam.data.body
    });
    const requester = RequestService.create(query.url);
    const response = await requester.call<IAddOverWorkData>(query);
  ...
  public async addOverWorkByGroup(
    params: AddOverloadRecordByGroupReqParam,
    schema: IJSONSchemaType
  ) {
    const validParam = Requester.validateParamWithData<
      AddOverloadRecordByGroupReqParam
    ...
    const query = this.rb.postOverWorkByGroupQuery({
      method: 'POST',
      headers: {},
      body: validParam.data.body
    });
